...
def deltaX_deltaY(bin_im):
    ...
    return deltaXY_list,XY0_list
    

def elliptic_fourier_descriptors(bin_im,N):
    
    deltaXY_list,XY0_list = deltaX_deltaY(bin_im)
    ...
    for deltaX, deltaY in deltaXY_list:
        ...
        efd[0,0] = efd[0,0] + XY0_list[counter][0]
        efd[0,2] = efd[0,2] + XY0_list[counter][1]
        efds[counter,:,:] = efd
        K_array[counter] = K
        T_array[counter] = T
        counter = counter + 1
...
def reconstruct(efds, K):
    
    N=len(efds)
    
    reconstructed = np.zeros((K,2))
    ...
    arg_grid = n_grid * t_grid / float(K)
